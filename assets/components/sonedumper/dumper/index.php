...
set_error_handler('sod_error_handler');
register_shutdown_function('sod_shutdown');
...
$SOD = new sOneDumper();
chdir(__DIR__);
$SOD->init(!empty($argc) && $argc > 1 ? $argv : false);
...
class sOneDumper
{
    ...
    public function __construct()
    {
        ...
        define('C_ERROR', 3);
        define('C_WARNING', 4);
        define('SOD_DEBUG', false);
        define('TIMER', array_sum(explode(' ', microtime())));
        define('V_SOD', 20011);
        define('V_PHP', sod_ver2int(phpversion()));
        $this->name = 'sOneDumper';
    }
    ...
    public function init($args = false)
    {
        ...
        if ($args) { // консольный режим
            ...
            if (!empty($_COOKIE['sod']) && preg_match('/^[\da-f]{32}$/', $_COOKIE['sod'])) {
                include $sfile;
                if (isset($SES[ $_COOKIE['sod'] ])) {
                    $auth      = true;
                    $this->CFG = $SES[ $_COOKIE['sod'] ]['cfg'];
                    $this->SES = &$SES;
            ...
            if (!$auth) {
                ...
                if ($auth) {
                    ...
                    $this->saveCFG();
                    if (V_PHP > 50200) {
                        setcookie('sod', $key, !empty($_POST['save']) ? time() + 31536000 : 0, '', '', false, true);
                    } else {
                        setcookie('sod', $key, !empty($_POST['save']) ? time() + 31536000 : 0, '', '', false);
                    }
                    header('Location: ./');
                    exit;
                }
            ...
            if (!$auth) {
                if (!empty($_POST['ajax'])) {
                    echo "sod.hideLoading();alert('Session not found');";
                    exit;
                }
    ...
    public function connect($host = null, $port = null, $user = null, $pass = null)
    {
        ...
        if ($GLOBALS['mysqlilink'] = mysqli_connect($this->CFG['my_host'] . ($this->CFG['my_host']{0} != ':' ? ":{$this->CFG['my_port']}" : ''), $this->CFG['my_user'], $this->CFG['my_pass'])) {
            if (V_PHP > 50202) {
                mysqli_set_charset($GLOBALS['mysqlilink'], 'utf8') or sod_my_error();
            } else {
                mysqli_query($GLOBALS['mysqlilink'], 'SET NAMES utf8') or sod_my_error();
            }
            define('V_MYSQL', sod_ver2int(mysqli_get_server_info($GLOBALS['mysqlilink'])));
        } else {
            define('V_MYSQL', 0);
            $this->error = 'sod.actions.tab_connects();alert(' . sod_esc(mysqli_error()) . ');';
        }
        $this->try = false;
    ...
    {
        // Тулбар
        $this->VAR['toolbar'] = sod_php2json(
            [
                ['backup', $this->LNG['tbar_backup'], 1, 0],
                ['restore', $this->LNG['tbar_restore'], 2, 0],
                ['|'],
                ['files', $this->LNG['tbar_files'], 3, 0],
                ['services', $this->LNG['tbar_services'], 5, 0],
                ['|'],
                ['createdb', $this->LNG['tbar_createdb'], 7, 0],
                ['connects', $this->LNG['tbar_connects'], 6, 0],
                ['|'],
                ['options', $this->LNG['tbar_options'], 4, 0],
                ['|'],
                ['exit', $this->LNG['tbar_exit'], 8, 0],
            ]
        );
        $this->db             = 'temp';
        ...
        $this->VAR['combos'] .= $this->getSavedJobs() . "sod.confirms = {$this->CFG['confirm']};sod.actions.dblist();";
    ...
    {
        $opts = !empty($opts) ? "{{$opt_name}:" . sod_php2json($opts) . '}' : "'{$opt_name}'";

        return "sod.addCombo('{$name}', '{$sel}', {$ico}, {$opts});\n";
    }
    ...
    public function ajax($req)
    {
        ...
        switch ($req['act']) {
            ...
            case 'load_files':
                ...
                break;
            case 'filelist':
                $res = "sod.clearOpt('files');sod.addOpt(" . sod_php2json(['files' => $this->getFileList()]) . ');';
                break;
            case 'dblist':
                $res = "sod.clearOpt('db');sod.addOpt(" . sod_php2json(['db' => $this->getDBList()]) . ");sod.combos.restore_db.select(0,'-');sod.combos.services_db.select(0,'-');sod.combos.backup_db.select(0,'-');";
                break;
            case 'load_connect':
			...
			z('backup_url').value = '{$CFG['backup_url']}', z('globstat').checked = {$CFG['globstat']}, z('charsets').value = '{$CFG['charsets']}', z('only_create').value = '{$CFG['only_create']}', z('auth').value = '{$CFG['auth']}', z('conf_import').checked = {$CFG['confirm']} & 1, z('conf_file').checked = {$CFG['confirm']} & 2, z('conf_db').checked = {$CFG['confirm']} & 4;sod.confirms = {$this->CFG['confirm']};";
                break;
            case 'save_options':
            ...
            case 'exit':
                setcookie('sod', '', 0);
                $res = 'top.location.href = ' . sod_esc($this->CFG['exitURL']) . ';';
                break;
        }
        echo $res;
    }
    ...
    {
        $r = mysqli_query($GLOBALS['mysqlilink'], 'DROP DATABASE `' . sod_esc($name, false) . '`') or sod_my_error();
        if ($r) {
            echo "sod.clearOpt('db');sod.addOpt(" . sod_php2json(['db' => $this->getDBList()]) . ");sod.combos.services_db.select(0,'-');";
        } else {
            echo 'alert(' . sod_esc(mysqli_error()) . ');';
        }
    }
    ...
    {
        $r = mysqli_query($GLOBALS['mysqlilink'], 'CREATE DATABASE `' . sod_esc($req['name'], false) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if ($r) {
            echo 'sod.addOpt(' . sod_php2json(['db' => [$req['name'] => "{$req['name']} (0)"]]) . ');';
        } else {
            sod_my_error();
        }
    }
    ...
    public function saveConnect($req)
    {
        ...
        if (V_MYSQL) {
            ...
            echo "sod.clearOpt('db');sod.clearOpt('charset');sod.clearOpt('collation');sod.addOpt(" . sod_php2json($tmp) . ");sod.combos.backup_db.select(0,'-');sod.combos.restore_db.select(0,'-');sod.combos.services_db.select(0,'-');sod.combos.backup_charset.select(0,'-');sod.combos.services_db.select(0,'-');sod.combos.db_charset.select(0,'-');";
        } else {
            echo $this->error;
        }
    }
    ...
    {
        if (isset($_COOKIE['sod'])) {
            $this->SES[ $_COOKIE['sod'] ] = ['cfg' => $this->CFG, 'time' => time(), 'lng' => $this->LNG['name']];
            $this->saveToFile('session.php', "<?php\n\$SES = " . var_export($this->SES, true) . ";\n" . '?>');
        }
    }
    ...
    public function runServices($job)
    {
        ...
        if (isset($serv[ $job['type'] ])) {
            ...
            $r = mysqli_query($GLOBALS['mysqlilink'], 'SHOW TABLE STATUS') or sod_my_error();
            if (!$r) {
                return;
            }
            $tables = [];
            while ($item = mysqli_fetch_assoc($r)) {
                ...
                if (sod_check($item['Name'], $object['TA'], $filter['TA'])) {
                    $tables[] = "`{$item['Name']}`";
                }
            }
            ...
            $r = mysqli_query($GLOBALS['mysqlilink'], $sql) or sod_my_error();
            if (!$r) {
                return;
            }
            $res = [];
            ...
            echo 'sod.result.add(' . sod_php2json($res) . ');';
        }
    }
        ...
        foreach ($objects as $t => $list) {
            ...
            foreach ($list as $item) {
                switch ($t) {
                    case 'TA':
                        $type = '';
                        if (sod_check($item[0], $object['TA'], $filter['TA'])) {
                            $type = empty($item[1]) ? 'TC' : 'TA';
                        } elseif (sod_check($item[0], $object['TC'], $filter['TC'])) {
                            $type = 'TC';
                        } else {
                            continue;
                        }
                        ...
                        break;
                    default:
                        if (sod_check($item, $object[ $t ], $filter[ $t ])) {
                            $todo[ $t ][] = [$t, $item];
                        }
                }
            }
    ...
    public function runRestoreJob($continue = false)
    {
        ...
        if ($this->JOB['strategy'] > 0) {
            $r = mysqli_query($GLOBALS['mysqlilink'], 'SHOW TABLES') or sod_my_error();
            while ($item = mysqli_fetch_row($r)) {
        ...
        while ($q = sod_read_sql($this->fh_tmp, $seek, $ei)) {
            if ($time_old < time()) {
                if (file_exists($this->JOB['file_stp'])) {
            ...
            switch ($q{0}) {
                ...
                case 'I':
                    if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
                        ...
                        mysqli_query($GLOBALS['mysqlilink'], "ALTER TABLE `{$tab}` DISABLE KEYS") or sod_my_error();
                        //if(!empty($this->JOB['autoinc'])) mysqli_query($GLOBALS['mysqlilink'], "ALTER TABLE `{$tab}` AUTO_INCREMENT = 1") or sod_my_error();
                        $ex = 1;
                    }
                    break;
                case 'C':
                    ...
                    if (preg_match("/\#\t(TC|TD|VI|PR|FU|TR|EV)`(.+?)`(([^_]+?)_.+?)?$/", $q, $m)) {
                        ...
                        if ($m[1] == 'TC') {
                            ...
                            if ($tc && ($this->JOB['strategy'] == 0 || isset($tab_exists[ $m[2] ]))) {
                                mysqli_query($GLOBALS['mysqlilink'], "{$tc} `{$m[2]}`") or sod_my_error();
                            }
                        } elseif ($m[1] == 'TD') {
                            $ei = 1;
                        } else {
                            ...
                            $this->rtl[7] = 0;
                            $this->rtl[8] = 0;
                            mysqli_query($GLOBALS['mysqlilink'], "DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sod_my_error();
        ...
        foreach ($this->JOB['todo']['TA'] as $tab) {
            ...
            mysqli_query($GLOBALS['mysqlilink'], "ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sod_my_error();
            $this->rtl[1] = time();
            $this->rtl[5] = $tab[1];
            fseek($this->fh_rtl, 0);
            fwrite($this->fh_rtl, implode("\t", $this->rtl));
    ...
    public function runRestoreJobForeign($continue = false)
    {
        ...
        $exit_time = $time_old + $this->CFG['time_web'] - 1;
        $delimiter = ';';
        while ($q = sod_read_sql($this->fh_tmp, $seek, $ei, $delimiter, $this->JOB['eol'])) {
            $q = ltrim($q);
            if (empty($q)) {
                break;
            }
    ...
    public function addBackupJob($job)
    {
        ...
        foreach ($queries as $query) {
            ...
            $r = mysqli_query($GLOBALS['mysqlilink'], 'SHOW ' . $query[0]) or sod_my_error();
            if (!$r) {
                continue;
            }
            $todo[ $t ]   = [];
            ...
            while ($item = mysqli_fetch_assoc($r)) {
                ...
                switch ($t) {
                    ...
                    case 'TC':
                        if (V_MYSQL > 40101 && is_null($item['Engine']) && preg_match('/^VIEW/i', $item['Comment'])) {
                            if (sod_check($n, $object['VI'], $filter['VI'])) {
                                $todo['VI']   = [];
                                $header['VI'] = [];
                            }
                            continue;
                        } elseif (sod_check($n, $object['TA'], $filter['TA'])) {
                            $engine = V_MYSQL > 40101 ? $item['Engine'] : $item['Type'];
                            $t      = in_array($engine, $only_create) ? 'TC' : 'TA';
                        } elseif (sod_check($n, $object['TC'], $filter['TC'])) {
                            $t            = 'TC';
                        ...
                        break;
                    default:
                        if (sod_check($n, $object[ $t ], $filter[ $t ])) {
        ...
        if (V_MYSQL > 50014 && (!empty($object['VI']) || !empty($filter['VI']))) {
            ...
            $r = mysqli_query($GLOBALS['mysqlilink'], "SELECT table_name, view_definition /*!50121 , collation_connection */ FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '{$this->JOB['db']}'") or sod_my_error();
        ...
        $this->JOB['todo']      = $todo;
        $this->saveJob($this->JOB['job'], $this->JOB);
        $fcache = implode('|', ['#SOD20', V_SOD, V_MYSQL, V_PHP, date('Y.m.d H:i:s'), $this->JOB['db'], $this->JOB['charset'], $tabs, $rows, sod_esc($this->JOB['comment'], false)]) . "\n";
    ...
    public function runBackupJob($continue = false)
    {
        ...
        if (V_MYSQL > 40101) {
            mysqli_query($GLOBALS['mysqlilink'], "SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') . "'") or sod_my_error();
        }
        $time_old  = time();
        $exit_time = $time_old + $this->CFG['time_web'] - 1;
                ...
                switch ($n[0]) {
                    ...
                    case 'TA':
                        ...
                        if ($n[0] == 'TC' || $this->rtl[7] == 0) {
                            ...
                            $r = mysqli_query($GLOBALS['mysqlilink'], "SHOW CREATE TABLE `{$n[1]}`") or sod_my_error();
                        ...
                        $notNum = [];
                        $r = mysqli_query($GLOBALS['mysqlilink'], "SHOW COLUMNS FROM `{$n[1]}`") or sod_my_error();
                        $fields = 0;
                    ...
                    default:
                        if (V_MYSQL < 50121 && $n[0] == 'TR') {
                            ...
                            $r = mysqli_query($GLOBALS['mysqlilink'], "SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sod_my_error();
                            $item = mysqli_fetch_assoc($r);
                            ...
                            $r = mysqli_query($GLOBALS['mysqlilink'], "SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sod_my_error();
                            $item = mysqli_fetch_assoc($r);
    ...
    public function setNames($collation)
    {
        ...
        if ($this->rtl[6] != $collation) {
            mysqli_query($GLOBALS['mysqlilink'], 'SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sod_my_error();
            /*if(!$this->rtl[7])*/
    ...
    public function getDBList()
    {
        ...
        if ($this->CFG['my_db']) {
            ...
            foreach ($tmp as $d) {
                $d         = trim($d);
                $items[]   = $qq . sod_esc($d, false) . $qq;
                $dbs[ $d ] = "{$d} (0)";
            }
        } else {
            $result = mysqli_query($GLOBALS['mysqlilink'], 'SHOW DATABASES') or sod_my_error();
            while ($item = mysqli_fetch_row($result)) {
                if ($item[0] == 'information_schema' || $item[0] == 'mysql' || $item[0] == 'performance_schema') {
                    continue;
                }
                $items[]         = $qq . sod_esc($item[0], false) . $qq;
                $dbs[ $item[0] ] = "{$item[0]} (0)";
            }
        }
        if (V_MYSQL < 50000) {
            foreach ($items as $item) {
                $tables = mysqli_query($GLOBALS['mysqlilink'], "SHOW TABLES FROM `{$item}`") or sod_my_error();
                if ($tables) {
            ...
            $result = mysqli_query($GLOBALS['mysqlilink'], "SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sod_my_error();
            while ($item = mysqli_fetch_row($result)) {
                if ($item[0] == 'information_schema' || $item[0] == 'mysql' || $item[0] == 'performance_schema') {
                    continue;
                }
    ...
    public function getCharsetList()
    {
        ...
        if (V_MYSQL > 40101) {
            ...
            $r = mysqli_query($GLOBALS['mysqlilink'], 'SHOW CHARACTER SET') or sod_my_error();
            if ($r) {
                while ($item = mysqli_fetch_assoc($r)) {
    ...
    public function getCollationList()
    {
        ...
        if (V_MYSQL > 40101) {
            ...
            $r = mysqli_query($GLOBALS['mysqlilink'], 'SHOW COLLATION') or sod_my_error();
            if ($r) {
                while ($item = mysqli_fetch_assoc($r)) {
                    if (empty($def_charsets) || preg_match($def_charsets, $item['Charset'])) {
    ...
    public function getFileObjects($tree, $name, $formatTree = true)
    {
        ...
        if (!is_readable($name)) {
            return "sod.tree.{$tree}.error(sod.lng('err_fopen'))";
        }
        ...
        return $formatTree ? $this->formatTree($tree, $objects) . "sod.comment.restore.value = '{$h[9]}';z('restore_savejob').disabled = z('restore_runjob').disabled = false;" : $objects;
    }
    ...
    public function formatTree($tree, &$objects)
    {
        ...
        foreach ($objects as $type => $o) {
            ...
            if ($type == 'TA') {
                ...
                $obj .= "[{$row},0," . sod_esc($info[ $type ][0]) . ',1,1,1],';
                ++$row;
                foreach ($o['*'] as $value) {
                    if (is_string($value)) {
                        if (count($o[ $value ]) > 1) {
                            ...
                            for ($i = 0, $l = count($o[ $value ]); $i < $l; ++$i) {
                                ...
                                $obj .= "[{$row},{$pid}," . sod_esc($o[ $value ][ $i ][0]) . ",2,{$checked},{$o[$value][$i][2]}],";
                                ++$row;
                            }
                        } else {
                            $value = $o[ $value ][0];
                ...
                $obj .= "[{$row},0," . sod_esc($info[ $type ][0]) . ",{$info[$type][1]},1,1],";
                $pid = $row++;
                ++$info[ $type ][1];
                for ($i = 0, $l = count($o); $i < $l; ++$i) {
                    $o[ $i ] = sod_esc($o[ $i ], false);
        ...
        return ($obj ? 'sod.tree.' . $tree . '.drawTree([' . substr_replace($obj, ']', -1) . ');' : "sod.tree.{$tree}.error(sod.lng('err_sod'));") . $add;
    }
    ...
    public function getSavedJobs()
    {
        ...
        return "sod.clearOpt('sj_backup');sod.clearOpt('sj_restore');sod.addOpt(" . sod_php2json($sj) . ');';
    }
    ...
    public function getFileListExtended()
    {
            ...
            while (false !== ($file = readdir($handle))) {
                if (preg_match("/^.+?\.sql(\.(gz|bz2))?$/", $file, $m)) {
                    ...
                    if (preg_match('/^(#SOD20\|.+?)\n/s', $temp, $m)) {
                        ...
                        $h       = explode('|', $m[1]);
                        $files[] = [$h[1], substr($h[3], 0, -3), $ext, $h[2], number_format((float)$h[4], 0, '', ' '), filesize((string)$this->CFG['backup_path'] . $file), 'SOD 1.0.x', $file];
                    } else {
        ...
        return 'sod.files.clear();sod.files.add(' . sod_php2json($files) . ');';
    }
...
function sod_read_sql($f, &$seek, $ei, $delimiter = "\t;", $eol = "\n")
{
    static $l = '';
    static $r = 0;
    $fs        = ftell($f);
...
function sod_check($n, $obj, $filt)
{
    return isset($obj[ $n ]) || ($filt && preg_match($filt, $n));
}

function sod_php2json($obj)
{
    if (count($obj) == 0) {
        return '[]';
    }
...
function sod_ver2int($ver)
{
    return preg_match("/^(\d+)\.(\d+)\.(\d+)/", $ver, $m) ? sprintf('%d%02d%02d', $m[1], $m[2], $m[3]) : 0;
}

function sod_error_handler($errno, $errmsg, $filename, $linenum, $vars)
{
    global $SOD;
    if ($SOD->try) {
        return;
    ...
    $str       = sod_esc("{$errortype[$errno]}: {$errmsg} ({$filename}:{$linenum})", false);
    if (SOD_DEBUG) {
        error_log("[index.php]\n{$str}\n", 3, 'backup/error.log');
    }

    if ($errno == 8 || $errno == 1024) {
        if (!$SOD->fh_log && !$SOD->fh_rtl) {
            echo isset($_POST['ajax']) ? "alert('" . ($str) . "');" : $str;
        } else {
            fwrite($SOD->fh_log, date('Y.m.d H:i:s') . "\t3\t{$str}\n");
        }
    } elseif ($errno < 1024) {
        $SOD->error = true;
        if (!$SOD->fh_log && !$SOD->fh_rtl) {
            ...
            $SOD->rtl[9] = 5;
            fseek($SOD->fh_rtl, 0);
            fwrite($SOD->fh_rtl, implode("\t", $SOD->rtl));
            fwrite($SOD->fh_log, date('Y.m.d H:i:s') . "\t4\t{$str}\n");
            unset($SOD->rtl);
        }
...
function sod_esc($str, $quoted = true)
{
    return $quoted ? "'" . addcslashes((string)$str, "\\\0\n\r\t\'") . "'" : addcslashes((string)$str, "\\\0\n\r\t\'");
}

function sod_my_error()
{
    trigger_error(mysqli_error(), E_USER_ERROR);
}

function sod_shutdown()
{
    global $SOD;
    if (isset($SOD->fh_rtl) && is_resource($SOD->fh_rtl) && !empty($SOD->rtl) && empty($SOD->error)) {
        $SOD->rtl[1] = time();
        if (!empty($SOD->JOB['file_stp']) && file_exists(dirname(__FILE__) . '/' . $SOD->JOB['file_stp'])) {
            $type        = file_get_contents(dirname(__FILE__) . '/' . $SOD->JOB['file_stp']);
            $SOD->rtl[9] = !empty($type) ? $type : 2;
        } else {
            $SOD->rtl[9] = 5;
        }
        fseek($SOD->fh_rtl, 0);
...
function sod_antimagic($arr)
{
    return is_array($arr) ? array_map('sod_antimagic', $arr) : stripslashes($arr);
}
